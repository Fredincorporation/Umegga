...
// clears the umega_laws table (if the migration has been applied).
const url = process.env.VITE_SUPABASE_URL;
const key = process.env.VITE_SUPABASE_ANON_KEY;
const h = { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' };

// 1) Clear world.activeLaws inside the umega_game_state snapshot.
...
if (!gs?.state) {
  ...
  const patchRes = await fetch(`${url}/rest/v1/umega_game_state?id=eq.global`, {
    method: 'PATCH',
...
// 2) Clear the umega_laws table (only if migration 004 has been applied).
const delRes = await fetch(`${url}/rest/v1/umega_laws?id=neq.__none__`, {
  method: 'DELETE',
  headers: { ...h, Prefer: 'return=representation' },
});
if (delRes.ok) {
  const removed = await delRes.json();
  console.log('umega_laws cleared, removed rows:', Array.isArray(removed) ? removed.length : removed);
} else {
  console.log('umega_laws not available yet:', delRes.status);
}

// 3) Verify.
const verify = await fetch(`${url}/rest/v1/umega_game_state?id=eq.global&select=state`, { headers: h });
